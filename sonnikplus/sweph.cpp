...
FILE *swi_fopen(int ifno, char *fname, char *ephepath, char *serr)
{
  ...
  strcpy(s1, ephepath);
  np = swi_cutstr(s1, (char*)PATH_SEPARATOR, cpos, 20);
  for (i = 0; i < np; i++) {
    strcpy(s, cpos[i]);
...
{ 
  ...
  struct plan_data *pdp;
  struct file_data *fdp = &swed.fidat[ifno];
  char *serr_file_damage = (char*)"Ephemeris file %s is damaged. ";
  ...
  double *xx, char *serr)
{
   ...
   ******************************************************/
  if (swed.fixfp == NULL) {
    if ((swed.fixfp = swi_fopen(SEI_FILE_FIXSTAR, (char*)SE_STARFILE, swed.ephepath, serr)) == NULL) {
      swed.is_old_starfile = TRUE;
      if ((swed.fixfp = swi_fopen(SEI_FILE_FIXSTAR, (char*)SE_STARFILE_OLD, swed.ephepath, NULL)) == NULL) {
	retc = ERR;
	goto return_err;
      }
    }
	...
	if (ipl == swed.fidat[SEI_FILE_ANY_AST].ipl[0])
	  ...
	  if (retc != ERR && retc != NOT_AVAILABLE)
        ...
        if (s[0] == '?' || isdigit((int) s[1])) {
          ...
          if ((fp = swi_fopen(-1, (char*)SE_ASTNAMFILE, swed.ephepath, NULL)) != NULL) {
...
char *FAR PASCAL_CONV swe_get_ayanamsa_name(int32 isidmode) 
{
  if (isidmode < SE_NSIDM_PREDEF)
    return (char*)ayanamsa_name[isidmode];
  return NULL;
}
